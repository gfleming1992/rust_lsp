...
import { parseBinaryLayer } from "../parsing/layerParser";
...
export async function discoverTestLayers(): Promise<string[]> {
  ...
  // Note: This is Vite-specific and only works in dev mode
  const glob = (import.meta as any).glob;
  if (!glob) {
    console.warn('import.meta.glob not available - not running in Vite dev mode');
    return [];
  }
  const modules = glob('./test-data/*.bin');
